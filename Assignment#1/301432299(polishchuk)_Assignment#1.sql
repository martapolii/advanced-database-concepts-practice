...
--Import/Create City Jail Database--
...
--1. Produce a list of sentencing information to include criminal ID, full name, sentence ID, sentence start date, 
   -- and the length in months of the sentence. The number of months should be shown as a whole number. 
   -- That start date should be displayed in the format “January 1, 2023” 
SELECT cr.criminal_id AS "Criminal ID", 
(cr.first || ' ' || cr.last) AS "Full Name", 
s.sentence_id AS "Sentence ID", 
TO_CHAR(s.start_date,'Month DD, YYYY') AS "Sentence Start Date", 
ROUND(ABS(MONTHS_BETWEEN(s.start_date,s.end_date))) AS "Length of Sentence (Months)" -- used "round" instead of trunc since whole numbers requested
   FROM criminals cr JOIN sentences s
   ON cr.criminal_id = s.criminal_id;
   
   
--2. List the name of each officer who has reported longer than the average number of crimes officers have reported.
SELECT (o.first || ' ' || o.last) AS "Officer Name",
    COUNT(co.crime_id) AS "Crimes Reported"
   FROM officers o
   JOIN crime_officers co 
      ON o.officer_id = co.officer_id
   GROUP BY o.first, o.last -- grouping officers by name
    HAVING COUNT(co.crime_id) > (SELECT AVG(cnt)--filtering for only those above avg  
    FROM (SELECT COUNT(*) AS cnt
          FROM crime_officers GROUP BY officer_id));
          

--3. List the names of all criminals who have committed less than average number of crimes and aren’t listed as violent offenders.
SELECT (cr.first || ' ' || cr.last) AS "Criminal Name",
   COUNT(c.crime_id) AS "Crimes Committed",
   cr.v_status AS "Violent Offender?"
FROM criminals cr
JOIN crimes c 
  ON cr.criminal_id = c.criminal_id
WHERE cr.v_status = 'N' -- filter for non-violent offenders
  GROUP BY cr.first, cr.last, cr.v_status
  HAVING COUNT(c.crime_id) < (SELECT AVG(cnt)
  FROM (SELECT COUNT(*) AS cnt
        FROM crimes GROUP BY criminal_id));
        


--4. List appeal information for each appeal that has longer than the average number of days between the filing and hearing dates.
SELECT appeal_id AS "Appeal ID",
    crime_id AS "Crime ID",
    filing_date AS "Filing Date",
    hearing_date AS "Hearing Date",
    (hearing_date - filing_date) AS "Days Between",
    status AS "Status"
FROM appeals
GROUP BY appeal_id, crime_id, filing_date, hearing_date, status
HAVING (hearing_date - filing_date) > (SELECT AVG(hearing_date - filing_date)
FROM appeals);


--5. List the names of probation officers who have had less than average number of criminals assigned.
SELECT (po.first || ' ' || po.last) AS "Probation Officer Name",
COUNT(s.criminal_id) AS "Criminals Assigned"
FROM prob_officers po
JOIN sentences s 
  ON po.prob_id = s.prob_id
GROUP BY po.first, po.last
HAVING COUNT(s.criminal_id) < (SELECT AVG(criminal_count)
    FROM (SELECT COUNT(criminal_id) AS criminal_count
        FROM sentences
        GROUP BY prob_id));


--6. List each crime that has had the least number of appeals recorded.
SELECT c.crime_id, c.classification, COUNT(a.appeal_id) AS "Number of Apeals"
FROM crimes c
LEFT JOIN appeals a --left join because regular join doesn't show crimes with 0 appeals
  ON c.crime_id = a.crime_id
GROUP BY c.crime_id, c.classification
HAVING COUNT(a.appeal_id) = (SELECT MIN(appeal_count)
    FROM (SELECT COUNT(a2.appeal_id) AS appeal_count
        FROM crimes c2 --need another alias for subquery
        LEFT JOIN appeals a2 
        ON c2.crime_id = a2.crime_id
        GROUP BY c2.crime_id));


--7. List the following information for all crimes that have a period longer than 10 days between the data charged and the hearing date: 
  -- crime ID, classification, date charged, hearing date, and the number of days between the date charged and the hearing date.

SELECT crime_id AS "Crime ID",
    classification AS "Classification",
    date_charged AS "Date Charged",
    hearing_date AS "Hearing Date",
    (hearing_date - date_charged) AS "Days Between"
FROM crimes

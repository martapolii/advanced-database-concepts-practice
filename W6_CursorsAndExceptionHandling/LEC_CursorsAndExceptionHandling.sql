/*
Week 6 - Cursors and Exception Handling

--------------------------------------------------------------------------------
 CURSORS 
--------------------------------------------------------------------------------
- cursors are used to process multiple data rows at once 
- unlike SELECT statements, which do 1 at a time
- run loops to accomplish this
- a cursor is a work area where rows are temporarily stored
    - queries are ran on the data, then the memory is freed up
    - prevents memory overloading 

*/

-- EXPLICIT CURSOR 
    -- declared explicitly to handle rows returned by a SELECT statement
...
BEGIN   
    ...
    LOOP
        FETCH cur_basket INTO rec_basket;
            ...
            -- calculating tax amount depending on if type is E or C 
...
-- CURSOR FOR LOOP 
    -- handles decclaration, fetch, ending loop automatically 
    -- for record locking: use FOR UPDATE and WHERE CURRENT 
DECLARE
   CURSOR cur_prod IS
        SELECT type, price
          FROM bb_product
...
BEGIN
  FOR rec_prod IN cur_prod LOOP
    ...
    DBMS_OUTPUT.PUT_LINE('ROWS UPADTED: '||SQL%ROWCOUNT);
   END LOOP;
-- need to COMMIT to udpdate changes permanently and release lock so other users can edit the data
COMMIT;
END;

-- CURSORS WITH PARAMETERS 
DECLARE
    -- p(arameter)_<parameter name>
...
-- CURSOR VARIABLE
    -- can be used for multiple query statements
DECLARE
...
-- BULK PROCESSING 
    -- processes groups of rows without context switching between SQL and PL/SQL 
    -- use FETCH with LIMIT clause 
    -- use FORALL with DML statements
-- (QUERY)
DECLARE
    CURSOR cur_item IS
        SELECT *
...
-- EXCEPTION HANDLERS 
-----------------------------------------------------------------------------
- exception handling is for handling errors
- have pre-defined exceptions, undefined, or user-defined
- need to add exception handlders to the code so the program is instructured on what to do when an error occurs,
  rather than the program just stopping execution/'crashing' from the users POV
  - can define a custom message to appear to the user explaining the error
  - OR can choose to execute code that will continue the program/run a specific function, etc
*/

-- PREDEFINED ERRORS
EXCEPTION 
    -- can add handlers for multiple exceptions in one exception body
    WHEN NO_DATA_FOUND THEN
        -- add custom msg to display when this error encountered
        DBMS_OUTPUT.PUT_LINE('You have no saved baskets!');
        -- can add anythihng to the error handler body (functions, etc)
    WHEN TOO_MANY_ROWS THEN
        DBMS_OUTPUT.PUT_LINE('A problem has occured in retrieving your saved basket.');
        DBMS_OUTPUT.PUT_LINE('Tech Support will be notified and contact you via email.');
END; 

-- UNDEFINED ERROR
    -- this will throw an error bc this id does not exist
    -- error number shown: -02292, integrity constraint
    -- can use this number when defining the exception handler for this exception specifically 

DELETE FROM bb_basket
    WHERE idBasket = 4; 
-- adding a handlder:
DECLARE 
  -- exception name declared
...
BEGIN
    DELETE FROM bb_basket
        WHERE idBasket = 4;
EXCEPTION
    -- if foreign key error occurs, this will run:
...
DECLARE
    ex_prod_update EXCEPTION; -- declare exception name
BEGIN
    UPDATE bb_product
    SET description = 'Mill grinder with 5 grind settings!'
    WHERE idProduct = 30;
  IF SQL%NOTFOUND THEN -- if no rows updated, run the bellow:
    RAISE ex_prod_update;
END IF;
-- exception handler will display a message: 
EXCEPTION
...
-- another example:
DECLARE
-- declaring business rules, ex: stock of an item
   lv_ordqty_num NUMBER(2) := 20; -- stock is 20
   lv_stock_num bb_product.stock%TYPE; --stores stock value from the table
   ex_prod_stk EXCEPTION;
BEGIN
   SELECT stock
   INTO lv_stock_num
   FROM bb_product
   WHERE idProduct = 2; -- stock is 20 for product with id 2
   IF lv_ordqty_num > lv_stock_num THEN -- if someone tries to order more than 20, error
      RAISE ex_prod_stk;
   END IF;
EXCEPTION
...
-- ADDTIONAL EXCEPTION CONCEPTS 
    -- WHEN OTHERS - catch all for errors you may not have anticipated
    -- SQLCODE - identifies error code
    -- SQLERRM -- identifies error message 
EXCEPTION
   WHEN NO_DATA_FOUND THEN
      DBMS_OUTPUT.PUT_LINE('You have no saved baskets!');
   WHEN OTHERS THEN
...
-- EXCEPTION PROPAGATION
    -- nested excdeptions
    -- start with inner then outer exception
EXCEPTION
   WHEN NO_DATA_FOUND THEN -- this is handled first
      DBMS_OUTPUT.PUT_LINE('No data error in nested block');
END;
   lv_junk_num := 3; -- variable assigned 3 
EXCEPTION
   WHEN OTHERS THEN -- handled second. catches any other errors 
...
-- Code to create database, ran before the above --
